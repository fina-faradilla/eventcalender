...
export default function AvailabilityGrid({ rows, events, mode }) {
    ...
    return <section className="availability-timeline card overflow-hidden" style={{ '--availability-slot-count': END_HOUR - START_HOUR, '--availability-slot-percent': `${100 / (END_HOUR - START_HOUR)}%`, '--availability-track-min-width': `${(END_HOUR - START_HOUR) * 84}px` }}>
        <div className="timeline-head timeline-grid"><div className="timeline-resource timeline-resource-heading">{isVenue ? 'Lokasi' : 'Anggota Staf'}</div><div className="timeline-hours">{hours.map((hour, index) => <div key={hour} className="timeline-hour" style={{ left: `${(index / (hours.length - 1)) * 100}%` }}>{String(hour).padStart(2, '0')}:00</div>)}</div></div>
        {rows.map(row => { const bookings = events.filter(event => matches(event, row)); return <div className="timeline-row timeline-grid" key={row.id}>
            <div className="timeline-resource"><b>{row.name}</b><span>{isVenue ? `Kapasitas: ${row.capacity || '—'}` : row.email || 'Staf acara'}</span><small className={`timeline-status ${row.available ? 'is-available' : 'is-occupied'}`}>{row.available ? 'Tersedia' : 'Terpakai'}</small></div>
