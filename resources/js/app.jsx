...
function Shell({ user, setUser }) {
    ...
    return <div className="min-h-screen lg:flex">
        <aside className={`app-sidebar fixed inset-y-0 left-0 z-40 flex h-screen w-[260px] shrink-0 flex-col px-4 py-8 transition-transform duration-200 ease-out lg:sticky lg:top-0 lg:translate-x-0 ${drawer ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="flex min-h-14 items-center justify-between"><div className="brand-lockup"><span className="brand-mark" aria-hidden="true"><CalendarDays size={22}/></span><div><b className="block font-bold text-ink">Technolife</b><span className="text-[11px] font-semibold uppercase tracking-wide text-neutral">{branding.subtitle}</span></div></div><button aria-label="Tutup menu" className="text-neutral lg:hidden" onClick={() => setDrawer(false)}><X/></button></div>
            <nav className="mt-8 flex-1 space-y-1 overflow-y-auto" aria-label="Navigasi utama">{(navigation[roleKey] || []).map(([id, label, Icon]) => <button key={id} onClick={() => { setPage(id); setDrawer(false); }} className={`nav-item ${page === id ? 'is-active' : ''}`}><Icon size={20}/><span>{label}</span></button>)}</nav>
            <div className="border-t border-line pt-4"><button onClick={() => { setPage(roleKey === 'STAFF' ? 'settings' : 'profile'); setDrawer(false); }} className={`nav-item ${page === (roleKey === 'STAFF' ? 'settings' : 'profile') ? 'is-active' : ''}`}><Settings size={20}/>Pengaturan</button><button onClick={logout} className="nav-item"><LogOut size={20}/>Keluar</button></div>
        </aside>
