...
<body>
    <div id="app">
        ...
        <main id="startup-fallback" style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;font-family:'Montserrat',sans-serif;text-align:center;color:#4A4A4A">
            <div><strong style="color:#DC143C;font-size:18px">Memuat aplikasi&hellip;</strong><p style="margin-top:8px;font-size:14px;color:#4A4A4A">Mohon tunggu sebentar.</p></div>
        </main>
    </div>
    @php
        $authUser = auth()->user() ? [
    ...
    <script>
        ...
        window.__showStartupError = function (error) {
            ...
            if (root && document.getElementById('startup-fallback')) {
                root.innerHTML = '<main id="application-error" style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;font-family:\'Montserrat\',sans-serif;text-align:center;color:#4A4A4A"><div><strong style="color:#DC143C;font-size:18px">Aplikasi gagal dimuat.</strong><p style="margin-top:8px;font-size:14px;color:#4A4A4A">Muat ulang halaman atau periksa koneksi Anda.</p></div></main>';
            }
        };
