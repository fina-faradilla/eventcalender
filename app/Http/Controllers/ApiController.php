...
use App\Enums\{EventStatus, ReminderType};
use App\Models\{AppNotification, Approval, Event, Reminder, User, Venue};
...
class ApiController extends Controller
{
    ...
    public function me(Request $request) { return $this->userPayload($request->user()); }
    ...
    public function bootstrap(Request $request)
    {
        ...
        } else $venues->each(fn ($venue) => $venue->setAttribute('available', true));
    ...
    public function events(Request $request)
    {
        ...
        if ($request->boolean('calendar')) $query->whereIn('status', EventStatus::confirmed());
        foreach (['status', 'venue_id', 'event_type'] as $field) if ($request->filled($field)) $query->where($field, $request->$field);
        if ($request->filled('pic')) $query->where('created_by', $request->pic);
        if ($request->filled('search')) $query->where('event_name', 'like', '%'.$request->search.'%');
    ...
    {
        if ($user->role === 'PIC') $query->where('created_by', $user->id);
        if ($user->role === 'STAFF') $query->whereHas('staff', fn ($staff) => $staff->where('users.id', $user->id));
    }
    ...
    public function store(Request $request)
    {
        ...
        return DB::transaction(function () use ($request, $data, $submit) {
            ...
            if ($submit) $this->notifyApprovers($event, false);
    ...
    public function update(Request $request, Event $event)
    {
        ...
        $submit = $request->boolean('submit'); $wasRejected = $event->status === EventStatus::Rejected->value;
        $data = $request->validate($this->rules($submit));
        return DB::transaction(function () use ($request, $data, $event, $submit, $wasRejected) {
            $this->conflicts($data, $event->id); $staff = $data['staff_ids'] ?? []; unset($data['staff_ids']);
            if ($submit) $data['status'] = EventStatus::Pending->value;
            $event->update($data); $event->staff()->sync($staff); $this->notifyAssignments($event, $staff);
            if ($submit && $wasRejected) Approval::create(['event_id' => $event->id, 'approver_id' => $request->user()->id, 'status' => 'RESUBMITTED']);
            if ($submit) { $this->notifyApprovers($event, $wasRejected); AppNotification::create(['user_id' => $event->created_by, 'event_id' => $event->id, 'title' => $wasRejected ? 'Event dikirim ulang' : 'Event dikirim', 'message' => "$event->event_name menunggu persetujuan."]); }
    ...
    {
        if (empty($data['event_date']) || empty($data['start_time']) || empty($data['end_time'])) return;
        ...
        if (! empty($data['venue_id']) && ($hit = (clone $overlap)->where('venue_id', $data['venue_id'])->with('venue')->first())) abort(409, ($hit->venue->name ?? 'Venue')." sudah digunakan pada {$hit->start_time} - {$hit->end_time}.");
        if (! empty($data['staff_ids']) && ($hit = (clone $overlap)->whereHas('staff', fn ($q) => $q->whereIn('users.id', $data['staff_ids']))->first())) abort(409, "Staff yang dipilih sudah memiliki event pada {$hit->start_time} - {$hit->end_time}.");
    }

    private function notifyApprovers(Event $event, bool $resubmitted): void
    {
        foreach (User::where('role', 'APPROVER')->where('is_active', true)->get() as $user) AppNotification::create(['user_id' => $user->id, 'event_id' => $event->id, 'title' => $resubmitted ? 'Event diajukan ulang' : 'Persetujuan baru', 'message' => "$event->event_name membutuhkan persetujuan Anda."]);
    }

    private function notifyAssignments(Event $event, array $staffIds): void
    {
        foreach (array_unique($staffIds) as $id) AppNotification::firstOrCreate(['user_id' => $id, 'event_id' => $event->id, 'title' => 'Penugasan event'], ['message' => "Anda ditugaskan pada $event->event_name."]);
    }
    ...
    {
        $user = $request->user(); $base = Event::query(); $this->scopeVisibleEvents($base, $user);
        $counts = fn (string $status) => (clone $base)->where('status', $status)->count();
        $common = ['upcoming' => (clone $base)->whereIn('status', EventStatus::confirmed())->whereDate('event_date', '>=', today())->count(), 'today' => (clone $base)->whereDate('event_date', today())->count()];
        return match ($user->role) {
            ...
            'STAFF' => $common + ['assigned' => (clone $base)->count(), 'completed' => $counts(EventStatus::Completed->value)],
    ...
    public function staffDashboard(Request $request)
    {
        ...
        return [
            'stats' => [
                ...
                'upcoming' => (clone $base)->whereIn('status', [EventStatus::Approved->value, EventStatus::Scheduled->value, EventStatus::Ongoing->value])->whereDate('event_date', '>=', today())->count(),
                'completed' => (clone $base)->where('status', EventStatus::Completed->value)->count(),
            ],
            'schedule' => (clone $base)->with(['venue', 'creator:id,name'])->whereDate('event_date', '>=', today())->orderBy('event_date')->orderBy('start_time')->limit(5)->get(),
    ...
    public function staffAssignments(Request $request)
    {
        ...
        if ($filter === 'upcoming') $query->whereIn('status', [EventStatus::Approved->value, EventStatus::Scheduled->value, EventStatus::Ongoing->value])->whereDate('event_date', '>=', today());
        if ($filter === 'confirmed') $query->whereIn('status', [EventStatus::Approved->value, EventStatus::Scheduled->value]);
        if ($filter === 'completed') $query->where('status', EventStatus::Completed->value);
    ...
    public function read(Request $request, AppNotification $notification) { abort_unless($notification->user_id === $request->user()->id, 403); $notification->update(['read_at' => now()]); return $notification; }

    public function remind(Request $request, Event $event, EventReminderService $reminders)
    {
        abort_unless($request->user()->role === 'ADMIN' || ($request->user()->role === 'PIC' && $event->created_by === $request->user()->id), 403);
        abort_unless(in_array($event->status, EventReminderService::eligibleStatuses(), true), 409, 'Pengingat hanya tersedia untuk event aktif yang terjadwal.');
        foreach (ReminderType::cases() as $type) $reminders->send($event, $type);
        return ['message' => 'Simulasi pengingat berhasil disimpan.', 'activity' => Reminder::where('event_id', $event->id)->latest()->get()];
    }

    public function users() { return User::latest()->get(['id', 'name', 'email', 'role', 'phone', 'is_active', 'created_at']); }
    ...
    public function venues() { return Venue::latest()->get(); }
    ...
    {
        if ($user->role === 'PIC') abort_unless($event->created_by === $user->id, 403, 'Anda tidak memiliki akses ke event ini.');
        if ($user->role === 'STAFF') abort_unless($event->staff()->where('users.id', $user->id)->exists(), 403, 'Anda tidak ditugaskan pada event ini.');
    }
}
